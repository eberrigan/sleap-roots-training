...
class TestMakeDatasetArtifact:
    ...
    def test_make_dataset_artifact_basic(self, mock_config, mock_wandb_init):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.datasets.wandb.Artifact', return_value=mock_artifact):
                ...
                mock_run.log_artifact.assert_called_once_with(mock_artifact, tags=["test", "dataset"])
                mock_run.finish.assert_called_once()
                assert result == mock_artifact
    
    @patch('sleap_roots_training.datasets.wandb.init')
    @patch('sleap_roots_training.datasets.CONFIG')
    def test_make_dataset_artifact_with_registry_link(self, mock_config, mock_wandb_init):
    ...
    @patch('sleap_roots_training.datasets.wandb.init')
    @patch('sleap_roots_training.datasets.CONFIG')
    def test_make_dataset_artifact_no_description_no_tags(self, mock_config, mock_wandb_init):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.datasets.wandb.Artifact', return_value=mock_artifact) as mock_artifact_class:
                result = make_dataset_artifact(
                    artifact_name="test_artifact",
                    dataset_path=str(dataset_path)
                )
    ...
    def test_make_dataset_artifact_exception_handling(self, mock_logging, mock_config, mock_wandb_init):
        ...
        with patch('sleap_roots_training.datasets.wandb.Artifact', side_effect=test_exception):
            with pytest.raises(Exception, match="Test exception"):
                make_dataset_artifact(
                    artifact_name="test_artifact",
                    dataset_path="/some/path/dataset.slp"
                )
            
            # Should still call run.finish() even on exception
            mock_run.finish.assert_called_once()
            mock_logging.error.assert_called_once_with(f"Error creating dataset artifact: {test_exception}")
    
    @patch('sleap_roots_training.datasets.wandb.init')
    @patch('sleap_roots_training.datasets.CONFIG')
    @patch('sleap_roots_training.datasets.logging')
    def test_make_dataset_artifact_logging_messages(self, mock_logging, mock_config, mock_wandb_init):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.datasets.wandb.Artifact', return_value=mock_artifact):
                ...
                # Check logging calls
                mock_logging.info.assert_any_call(f"Dataset artifact created: test_artifact from {dataset_path}.")
                mock_logging.info.assert_any_call(f"Linking test_artifact to registry test_entity-org/wandb-registry-test_registry/test_collection.")
    ...
    def test_make_dataset_artifact_path_conversion(self, mock_config, mock_wandb_init):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.datasets.wandb.Artifact', return_value=mock_artifact):
                ...
                assert mock_artifact.metadata["data_path"] == Path(dataset_path_str).as_posix()
                mock_artifact.add_file.assert_called_once_with(
