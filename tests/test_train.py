...
class TestExecuteTraining:
    ...
    def test_execute_training_failure(self, mock_print, mock_subprocess_run):
        ...
        mock_print.assert_any_call(f"Executing: {command}")
        mock_print.assert_any_call("Error executing training command: Training failed with error")
...
class TestEvaluateModelAndGenerateVisuals:
    ...
    def test_evaluate_model_and_generate_visuals(self, mock_figure, mock_histplot, 
                                                 mock_close, mock_savefig, mock_load_metrics):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert visualizations["distance_histogram"] == model_dir / "distance_histogram.png"
            
            mock_load_metrics.assert_called_once_with(model_dir.as_posix(), split="test")
