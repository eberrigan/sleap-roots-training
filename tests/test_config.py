...
class TestConfig:
    ...
    def test_default_config_structure(self):
        ...
        expected_keys = {
            "project_name", "entity_name", "experiment_name", 
            "registry", "collection_name", "job_type"
        }
        assert set(DEFAULT_CONFIG.keys()) == expected_keys
        assert DEFAULT_CONFIG["project_name"] == "sleap-roots"
        assert DEFAULT_CONFIG["entity_name"] == "eberrigan-salk-institute-for-biological-studies"
    ...
    def test_update_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.config.CONFIG_PATH', test_config_path):
                with patch('sleap_roots_training.config.CONFIG', DEFAULT_CONFIG.copy()) as mock_config:
    ...
    def test_config_path_points_to_correct_location(self):
        ...
        expected_path = Path(__file__).parent.parent / "sleap_roots_training" / "config.yaml"
    ...
    def test_create_default_config_prints_message(self, mock_print):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.config.CONFIG_PATH', test_config_path):
                ...
                mock_print.assert_called_once_with(f"Default config.yaml created at {test_config_path}")
    ...
    def test_save_config_prints_message(self, mock_print):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.config.CONFIG_PATH', test_config_path):
                ...
                mock_print.assert_called_once_with(f"Configuration updated successfully at {test_config_path}.")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('sleap_roots_training.config.CONFIG_PATH', test_config_path):
                with patch('sleap_roots_training.config.CONFIG', {"modified": "value"}):
                    ...
                    mock_print.assert_called_with("Configuration has been reset to default values.")